...
uses
  ...
  DateVk, SystemConsts, Math, DocSqlManager, RtcQueryDataSet, RtcLog,
   rtcInfo, rtcConn, rtcDataCli, rtcCliModule, ClientDocSqlManager, CommonInterface ;
...
type
  ...
  private
    ...
    FDQueryAttributesOfGroup: TRtcQueryDataSet;
    FRootIdGroup: Int64;
    ...
    procedure SetRootIdGroup(const Value: Int64);
  protected
  ...
  public
    ...
    procedure PrepareRtcFunction(const operation:TDocOperation; clientModule: TRtcClientModule; sqlManaget: TClientDocSqlManager);
    ...
    property RootIdGroup: Int64 read FRootIdGroup write SetRootIdGroup;
  end;
...
uses uLog, DmMainRtc, uDocDescription, sqldescription, rtc.DocBinding, frameObjectsGr,Vcl.Controls, fmVkDocDialog;
{$R *.dfm}
    ...
    begin
      ...
      case FDQueryAttributesOfGroup.FieldByName(FLD_ATTRIBUTETYPE).AsInteger of
        ...
        begin
          ...
          TDocMEditBoxBindingDescription.GetDocMEditBoxBindingDescription('TObjectsGrFrame',InitBeforeSelect),_set_name);
        end;
        TA_ACCOUNT:
        begin
        end;
...
//  MainDm.LinkWithQuery(FDQueryAttributesOfGroup,DmMain.FDTransactionRead);
  FDocEditFunctionName := 'rtcObjectEdit';

  FDQueryAttributesOfGroup := MainRtcDm.NewRtcQueryDataSet;
  with FDQueryAttributesOfGroup do
  begin
    SQL.Clear;
...
//  SqlManager.AdditionalList.Add(FAdditionalSqlManager);
  ...
  OnPrepareRtcFunction := PrepareRtcFunction;
end;
...
begin
  ...
  begin
    if AStatus= usInserted then
      DocVariableList.VarByName('IDGROUP').AsLargeInt := FRtcQueryDataSet.ParamByName('idgroup').AsLargeInt;
  end;
end;
...
begin
  ...
  begin
    if AInsert then
    begin
      _kodg := FRtcQueryDataSet.ParamByName('idgroup').AsLargeInt;
      if _kodg=0 then
        raise Exception.Create('_kodg = 0');
      DocVariableList.VarByName('idgroup').AsLargeInt :=  _kodg;
      DocVariableList.VarByName('idobject').AsLargeInt :=  MainRtcDm.Gen_Id('IDOBJECT');
...
begin
  Result := TObjectsDm.Create(MainRtcDm);
end;
...
///  ListBoolFields.Clear;
  ...
  try
    ...
    FDQueryAttributesOfGroup.ParamByName('iduser').AsLargeInt := MainRtcDm.UserInfo.id_user;
    FDQueryAttributesOfGroup.ParamByName('iduatype').AsLargeInt := MainRtcDm.UserAccessTypes.VarByName('USERACCESS_ATTRIBUTES').AsInteger;
...
procedure TObjectsDm.InitBeforeSelect(Sender: TObject);
var fm: TVkDocDialogFm;
    binding: TCustomDocFmVkVariableBinding;
    idx: Integer;
    v: TVkVariable;
begin
  fm := TVkDocDialogFm.GetParrentForm((TWinControl(Sender).Parent));
  idx := fm.BindingList.IndexOfControl(TWinControl(Sender));
  if (idx > -1) then
  begin
    binding := TCustomDocFmVkVariableBinding(fm.BindingList.Items[idx]);
    with TCustomDocFmVkVariableBinding(binding).DocMEditBox do
    begin
      v := binding.params.FindVkVariable('IDGROUP');
      if Assigned(v) then
         TObjectsGrFrame(DocFm.FrameDoc).RootIdGroup := v.AsLargeInt;
    end;
  end;
end;
  ...
  begin
    ...
    begin
      ...
      case FDQueryAttributesOfGroup.FieldByName(FLD_ATTRIBUTETYPE).AsInteger of
        ...
        TA_OBJECT,TA_GROUP:
        begin
          binding := TCustomDocFmVkVariableBinding(_Item);
          binding.params.Clear;
          if Assigned(binding) and Assigned(binding.DocMEditBox)then
          begin
              binding.params.CreateVkVariable('IDGROUP',
              CoalEsce(MainRtcDm.QueryValue('SELECT idgroup FROM attributelist WHERE idattribute=:idattribute',
...
procedure TObjectsDm.Open;
begin
  if ParamsPrepared then
  begin
    //MemTableEhDoc.Close;
    FRtcQueryDataSet.DsMemTableEh := MemTableEhDoc;
    DocVariableList.Clear;
    //Prepare(FRootIdGroup);
    ...
    try
      ...
      begin
        ...
        FRtcQueryDataSet.ParamByName('idobject').AsLargeInt := FRootIdGroup;
      end
      else
      begin
        SqlManager.SelectSQL.Text := GetSelectFromObjects(FRootIdGroup);
        FRtcQueryDataSet.SQL.Text := SqlManager.SelectSQL.Text;
        FRtcQueryDataSet.ParamByName('idgroup').AsLargeInt := FRootIdGroup;
      end;
      FRtcQueryDataSet.Open;
      if ObjectsTypeDm=tdmoGroups then
        if Assigned(MemTableEhDoc.RecView) then
          MemTableEhDoc.RecView.NodeExpanded := true;
     if (SqlManager.KeyFieldsList.IndexOf('IDGROUP')=-1) then
     begin
      //SqlManager.KeyFieldsList.Add('IDGROUP');
        XLog('Ok');
     end;
      ...
      on E: Exception do
      begin
        LogMessage(' DmObjects:'+#13#10+E.Message+#13#10+FRtcQueryDataSet.SQL.Text);
      end;
    end;
  end;
end;
...
procedure TObjectsDm.PrepareRtcFunction(const operation: TDocOperation; clientModule: TRtcClientModule;
  sqlManaget: TClientDocSqlManager);
begin
   with ClientModule do
   begin
     if Assigned(Param.asRecord['PARAMS'].asRecord['KEY']) then
       Param.asRecord['PARAMS'].asRecord['KEY'].asValue['IDGROUP'] :=
         DocVariableList['IDGROUP'].Value;
   end;
...
procedure TObjectsDm.SetRootIdGroup(const Value: Int64);
begin
  FRootIdGroup := Value;
  Prepare(Value);
end;
