...
uses
  ...
  frameObjectsItems, Vcl.StdCtrls, VkVariable, VariantUtils, rtc.fmcustomdoc, EhLibVCL;
...
{$R *.dfm}
uses systemconsts, DmMainRtc, ActionManagerDescription;
...
procedure TObjectsGrFrame.DataSource1DataChange(Sender: TObject; Field: TField);
begin
  inherited;
...
begin
  inherited;
  if Assigned(FObjectsDm) and FObjectsDm.MemTableEhDoc.Active then
  begin
...
begin
  ...
  begin
    ...
    FObjectsDm.DocVariableList.VarByName('idobject').AsLargeInt :=  FObjectsDm.DmMain.Gen_Id('IDOBJECT');
...
begin
        ...
        'SELECT name FROM objects WHERE idobject=:idobject',[FRootIdGroup]),'')
...
function TObjectsGrFrame.getSetUpKeyName: String;
begin
  Result := inherited;
//  Result := Result+IntToStr(FCurrentKodg);
end;
...
//    v := TVkVariableCollection.Create(Self);
    try
      v := MainRtcDm.QueryValue('SELECT idgroup, isgroup FROM objects WHERE idobject=:idobject',
        [AValue.Items[0].AsLargeInt]);
      if VarIsArray(v) and (VarArrayHighBound(v,1)>0) then
      begin
        if (v[1] = true) then
        begin
        ...
        begin
          ...
          try
            _nv.CreateVkVariable('idobject0',v[0]);
...
begin
  ...
  begin
    ...
    FTypeGroup := MainRtcDm.getTypeGroup(FRootIdGroup);
    if not Prepared then
    begin
      FObjectsDm.RootIdGroup := Value;
      ConfigureEdit;
      SetObjectsItemsVisible( not FObjectsDm.MemTableEhDoc.RecView.NodeHasChildren);
      GetParentForm.Caption := ifVarEmpty(MainRtcDm.QueryValue('SELECT name FROM objects WHERE idobject=:idobject',
       [FObjectsDm.RootIdGroup]),'');
    end;
  end;
end;
...
var
    varList: TVkVariableCollection;
begin
  varList := TVkVariableCollection.Create(self);
  varList.CreateVkVariable('idobject',FObjectsDm.MemTableEhDoc.FieldByName('idobject').AsLargeInt);
  try
    TDocFrame.ViewFrame('TAttributesOfGroupFrame',varList);
  finally
//    varList.Free;
  end;
end;
