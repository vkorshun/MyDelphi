...
begin
  ...
  begin
    if Avar.Count=1 then
      Result := IfVarEmpty(
      MainDm.QueryValue('SELECT name FROM attributelist WHERE idattribute=:idattribute',[Avar.Items[0].AsLargeInt]),'')
    else
      Result := 'not defined';
  end;
end;
...
begin
  Result := 'Аттрибуты объектов аналитического учета';
end;

function TAttributesFrameOKU.GetCaption: String;
begin
  Result := 'Аттрибуты объектов количественного учета';
end;
