...
{ FireDAC Advantage Database Server driver base classes }
{                                                       }
...
{$ENDIF}
    ...
    class function DumpField(AField: PAceChar): String;
...
{$ENDIF}
    ...
    function GetTransactionCount: LongWord;
...
{$ENDIF}
  ...
  public
    ...
    property FieldType: Word read FFieldType;
    property MemoType: Word read FMemoType;
...
{$IFDEF FireDAC_MONITOR}
    ...
    function DumpValue: UnicodeString;
...
{$ENDIF}
  ...
  private
    ...
    function GetLastAutoInc: LongWord;
...
{$ENDIF}
  public
    ...
    // RO
    property LastAutoInc: LongWord read GetLastAutoInc;
  ...
  private
    ...
    function GetRecNo: LongWord;
    procedure SetRecNo(const AValue: LongWord);
...
{$ENDIF}
  ...
  public
    ...
    procedure Recall;
    procedure Post;
    procedure Lock(ARec: UNSIGNED32);
    ...
    property RecordCount: LongWord read GetRecordCount;
    property RecNo: LongWord read GetRecNo write SetRecNo;
    // Object props
...
begin
  ...
  // default search path.
  strAliasSearchPath := FDReadEnvValue('adsini_path');
...
begin
  ...
  if not FileExists(Result) then begin
    ...
    if FileExists(FDReadEnvValue('HOME') + '/.ads.ini') then
      Result := FDReadEnvValue('HOME') + '/.ads.ini'
    else
      Result := '/etc/ads.ini';
  end;
end;
...
begin
  try
    oIni := TFDIniFile.Create(AliasFileName);
    try
  ...
  begin
    ...
    if i1 <> 0 then begin
      if AMarker2 = '' then
        ...
        i2 := FDPosEx(AMarker2, sErr, i1 + Length(AMarker1));
      if i2 <> 0 then
...
begin
  ...
  inherited Create(er_FD_ADSGeneral,
    FDExceptionLayers([S_FD_LPhys, S_FD_ADSId]) + ' ' + String(sErr));

  AppendError(1, ACode, String(sErr), sObj, eKind, iErrPos - 1, -1);
{$IFDEF FireDAC_MONITOR}
...
{-------------------------------------------------------------------------------}
function TADSConnection.GetTransactionCount: LongWord;
...
{-------------------------------------------------------------------------------}
function TADSStatement.GetLastAutoInc: LongWord;
...
{-------------------------------------------------------------------------------}
function TADSStatement.GetRecordCount: LongWord;
...
{-------------------------------------------------------------------------------}
procedure TADSStatement.SetTimeout(const AValue: LongWord);
...
{$IFDEF FireDAC_MONITOR}
  ...
  begin
    Trace(sAdsStmtSetTablePassword, ['hStatement', Handle,
      'pucTableName', ATableName, 'pucPassword', '*****']);
  end;
{$ENDIF}
...
{-------------------------------------------------------------------------------}
function TADSBLOBStream.Read(var Buffer; Count: Integer): Longint;
var
  iCount: UNSIGNED32;
  iRes: UNSIGNED32;
...
{-------------------------------------------------------------------------------}
function TADSBLOBStream.Write(const Buffer; Count: Integer): Longint;
var
...
{$ENDIF}
  ...
  FObj.Check(FObj.Lib.FAdsSetBinary(FObj.Handle, FVariable.FField, usBinaryType,
    Count, FPosition, PAceBinary(@Buffer), Count));
  FPosition := FPosition + Count;
  Result := Count;
end;

{-------------------------------------------------------------------------------}
function TADSBLOBStream.Seek(Offset: Integer; Origin: Word): Longint;
var
  ulLength: UNSIGNED32;
  iRes: UNSIGNED32;
  ...
  ADS_COMPACTDATE:
    begin
      ...
      if lDate = 0 then
        ...
        PLongWord(AValue)^ := lDate - JULIAN_TO_DELPHI_DATE;
        ASize := SizeOf(LongWord);
      end;
    end;
  ADS_TIME:
    begin
      FObj.Check(FObj.Lib.FAdsGetMilliseconds(FObj.Handle, FField, @lTime));
      PLongWord(AValue)^ := lTime;
      ASize := SizeOf(LongWord);
    end;
  ADS_TIMESTAMP,
  ADS_MODTIME:
    begin
      ...
      if lDate = 0 then
        ...
        FObj.Check(FObj.Lib.FAdsGetMilliseconds(FObj.Handle, FField, @lTime));
        PSQLTimeStamp(AValue)^ := FDDateTime2SQLTimeStamp(
...
begin
  ...
  ADS_DATE,
  ADS_COMPACTDATE:
    FObj.Check(FObj.Lib.FAdsSetJulian(FObj.Handle, FField, PLongWord(AValue)^ + JULIAN_TO_DELPHI_DATE));
  ADS_TIME:
    FObj.Check(FObj.Lib.FAdsSetMilliseconds(FObj.Handle, FField, PLongWord(AValue)^));
  ADS_TIMESTAMP,
  ADS_MODTIME:
    begin
...
{-------------------------------------------------------------------------------}
function TADSVariable.DumpValue: UnicodeString;
var
  aBuff: array [0 .. 1023] of AceChar;
  aBytes: array [0 .. 255] of Byte;
  pData: Pointer;
  ...
  if IsNull then
    ...
    ADS_NUMERIC:
      begin
        ...
        SetString(s, PChar(@aBytes[0]), iSize);
    ...
    ADS_SHORTINT:
      Result := IntToStr(PSIGNED16(pData)^);
    ADS_LONGLONG,
    ADS_ROWVERSION:
      Result := IntToStr(PInt64(pData)^);
    ADS_DATE,
    ADS_COMPACTDATE:
      Result := DateToStr(FDDate2DateTime(PLongWord(pData)^));
    ADS_TIME:
      Result := TimeToStr(FDTime2DateTime(PLongWord(pData)^));
    ADS_TIMESTAMP,
    ADS_MODTIME:
      Result := DateTimeToStr(FDSQLTimeStamp2DateTime(PSQLTimeStamp(pData)^));
    ADS_STRING,
    ADS_CISTRING,
    ADS_VARCHAR,
    ADS_VARCHAR_FOX:
      if iSize > 1024 then begin
        s := FObj.Encoder.Decode(pData, 1024);
        Result := UnicodeString('(truncated at 1024) ''' + s + ' ...''');
      end
      else begin
        s := FObj.Encoder.Decode(pData, iSize);
        Result := UnicodeString('''' + s + '''');
      end;
    ADS_NCHAR,
    ADS_NVARCHAR:
      if iSize > 1024 then begin
        SetString(sw, PWideChar(pData), 1024);
        Result := '(truncated at 1024) ''' + sw + ' ...''';
      end
      else begin
        SetString(sw, PWideChar(pData), iSize);
        Result := '''' + sw + '''';
      end;
    ADS_RAW,
    ADS_VARBINARY_FOX,
    ADS_SYSTEM_FIELD,
    ADS_TYPE_UNKNOWN:
      if iSize > 512 then begin
        s := FDBin2Hex(pData, 512);
        Result := UnicodeString('(truncated at 512) ' + s + ' ...');
      end
      else begin
        s := FDBin2Hex(pData, iSize);
        Result := UnicodeString(s);
      end;
    ADS_BINARY,
    ADS_IMAGE,
...
begin
  Result := nil;
  for i := 0 to FList.Count - 1 do
    if CompareText(AName, String(TADSVariable(FList[i]).Name)) = 0 then begin
...
{-------------------------------------------------------------------------------}
function TADSCursor.GetRecordCount: LongWord;
...
{-------------------------------------------------------------------------------}
function TADSCursor.GetRecNo: LongWord;
...
{$IFDEF FireDAC_MONITOR}
  if Tracing then Trace1;
{$ENDIF}
  Check(Lib.FAdsGetRecordNum(Handle, ADS_RESPECTFILTERS, @Result));
end;

{-------------------------------------------------------------------------------}
procedure TADSCursor.SetRecNo(const AValue: LongWord);
...
{-------------------------------------------------------------------------------}
procedure TADSCursor.Lock(ARec: UNSIGNED32);
...
{$IFDEF FireDAC_MONITOR}
  if Tracing then Trace1;
{$ENDIF}
  Check(Lib.FAdsLockRecord(Handle, ARec));
end;
...
{$IFDEF FireDAC_MONITOR}
  if Tracing then Trace1;
{$ENDIF}
  FLib.Check(FLib.FAdsOpenTable(Connection.Handle, PAceChar(TableName),
    PAceChar(Alias), usTableType, CharType, usLockType, CheckRights, Options, @Handle));
  UpdateFieldsNum;
end;
...
{$IFDEF FireDAC_MONITOR}
  if Tracing then Trace1;
{$ENDIF}
  FLib.Check(FLib.FAdsOpenTable101(Connection.Handle, PAceChar(TableName), @Handle));
  UpdateFieldsNum;
end;
...
var
  sTableName, sAlias, sFields: TFDByteString;
begin
  usLockType := ADSLockType2Int(LockType);
  usTableType := ADSTableType2Int(TableType);
{$IFDEF FireDAC_MONITOR}
  if Tracing then Trace1;
{$ENDIF}
  sTableName := Encoder.Encode(TableName);
  sAlias := Encoder.Encode(Alias);
  sFields := Encoder.Encode(AFields);
  FLib.Check(FLib.FAdsCreateTable(Connection.Handle, PAceChar(PByte(sTableName)),
    PAceChar(PByte(sAlias)), usTableType, CharType, usLockType, CheckRights,
    AMemoSize, PAceChar(PByte(sFields)), @Handle));
  UpdateFieldsNum;
end;
...
var
  sFileName, sTag,
  sKeyExpr, sCondition, sWhile: TFDByteString;
begin
{$IFDEF FireDAC_MONITOR}
  if Tracing then Trace1;
{$ENDIF}
  sFileName := Encoder.Encode(AFileName);
  sTag := Encoder.Encode(ATag);
  sKeyExpr := Encoder.Encode(AKeyExpr);
  sCondition := Encoder.Encode(AKeyExpr);
  sWhile := Encoder.Encode(AWhile);
  Check(Lib.FAdsCreateIndex(Handle, PAceChar(PByte(sFileName)),
    PAceChar(PByte(sTag)), PAceChar(PByte(sKeyExpr)),
    PAceChar(PByte(sCondition)), PAceChar(PByte(sWhile)), AOptions,
    @hIndex));
end;
...
{-------------------------------------------------------------------------------}
procedure TADSDictionary.SetLongWord(AIndex: Integer;
  const AValue: UNSIGNED32);
...
{-------------------------------------------------------------------------------}
procedure TADSDictionary.SetString(AIndex: Integer;
  const AValue: String);
...
{$ENDIF}
  sb := Encoder.Encode(AValue);
  Check(Lib.FAdsDDSetDatabaseProperty(Handle, AIndex, PAceChar(PByte(sb)),
    Length(sb)));
end;
