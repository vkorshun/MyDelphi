...
type
  ...
  private
    ...
    procedure AddOption(var AStr: String; const AName, AValue: String);
  protected
  ...
  TFDADSBackupOptions = set of (boDontOverwrite, boMetaOnly, boPrepareDiff, boDiff);
  [ComponentPlatformsAttribute(pidWin32 or pidWin64)]
  ...
  TFDADSRestoreOptions = set of (roDontOverwrite, roMetaOnly);
  [ComponentPlatformsAttribute(pidWin32 or pidWin64)]
...
{$IFDEF MSWINDOWS}
  Winapi.Windows,
{$ENDIF}
  System.Variants, System.StrUtils, Data.DB,
...
{$ENDIF}
  ...
  private
    ...
    procedure SetupStatement(AStmt: TADSStatement);
    ...
    procedure SetParamValue(AFmtOpts: TFDFormatOptions; AParam: TFDParam;
      AAdsParam: TADSVariable; ApInfo: PFDAdsParInfoRec; AArrIndex: Integer);
    ...
    function AllocStatement: TADSStatement;
    procedure ReleaseStatement(var AStmt: TADSStatement);
  protected
...
begin
  Result := (FDefaultPath <> '') and (CompareText(String(FDefaultPath), GetCurrentDir) <> 0);
end;
...
begin
  ...
  if UserName <> '' then
    sConn := sConn + ';User_Name=' + UserName;
  if Password <> '' then
    sConn := sConn + ';Password=' + Password;
  FDPhysManager().CreateConnection(sConn, AConn);
...
begin
  FInclude.Assign(AValue);
end;

{-------------------------------------------------------------------------------}
procedure TFDADSBackupRestore.SetExclude(const AValue: TStrings);
begin
  FExclude.Assign(AValue);
end;

{-------------------------------------------------------------------------------}
procedure TFDADSBackupRestore.SetTableTypeMap(const AValue: TStrings);
begin
  FTableTypeMap.Assign(AValue);
end;

{-------------------------------------------------------------------------------}
procedure TFDADSBackupRestore.AddOption(var AStr: String; const AName, AValue: String);
begin
  if AStr <> '' then
    AStr := AStr + ';';
  AStr := AStr + AName;
...
begin
  ...
  if boDontOverwrite in Options then
  ...
  try
    if oNames.Count = 0 then begin
      ...
      try
        ...
        oMetaCmd.TableKinds := [tkTable, tkTempTable, tkLocalTable];
    ...
    try
      if not oDict.HasDictionary then
        sPath := FDNormPath(String(TADSConnection(oConn.CliObj).ConnectionPath))
      else
        sPath := '';
    finally
      FDFree(oDict);
...
{-------------------------------------------------------------------------------}
function TFDPhysADSDriver.GetConnParamCount(AKeys: TStrings): Integer;
begin
  Result := inherited GetConnParamCount(AKeys) + 13;
end;

{-------------------------------------------------------------------------------}
procedure TFDPhysADSDriver.GetConnParams(AKeys: TStrings; AIndex: Integer;
...
begin
  ...
  if AIndex < inherited GetConnParamCount(AKeys) then begin
    ...
    if AName = S_FD_ConnParam_Common_Database then begin
      ALoginIndex := 2;
      AType := '@F:Advantage Dictionary|*.add';
    end;
  end
  else begin
    ...
    0:
      begin
        AName := S_FD_ConnParam_ADS_Alias;
        Employ;
        try
    ...
    1:
      begin
        AName := S_FD_ConnParam_ADS_ServerTypes;
        AType := S_FD_Remote + ';' + S_FD_Local + ';' + S_FD_Internet;
      end;
    2:
      begin
        AName := S_FD_ConnParam_ADS_Protocol;
        AType := S_FD_UDP + ';' + S_FD_IPX + ';' + S_FD_TCPIP + ';' + S_FD_TLS;
      end;
    3:
      begin
    ...
    5:
      begin
        ...
        AType := C_AdsCharTypeMap;
        ADefVal := 'ANSI';
      end;
    6:
      begin
        AName := S_FD_ConnParam_ADS_Compress;
        AType := S_FD_Internet + ';' + S_FD_Always + ';' + S_FD_Never;
      end;
    7:
      begin
        AName := S_FD_ConnParam_ADS_TableType;
        AType := S_FD_ADT + ';' + S_FD_VFP + ';' + S_FD_CDX + ';' + S_FD_NTX;
      end;
    8:
      begin
        AName := S_FD_ConnParam_ADS_TablePassword;
        AType := '@S';
      end;
    9:
      begin
        AName := S_FD_ConnParam_ADS_Locking;
        AType := S_FD_Proprietary + ';' + S_FD_Compatible;
      end;
    10:
      begin
        AName := S_FD_ConnParam_ADS_Advanced;
        AType := '@S';
      end;
    11:
      begin
...
{-------------------------------------------------------------------------------}
function TFDPhysADSConnection.InternalCreateEvent(
...
begin
  Result := TFDPhysADSMetadata.Create(Self, FServerVersion,
    TFDPhysADSDriver(DriverObj).FLib.Version);
end;
...
{-------------------------------------------------------------------------------}
function TFDPhysADSConnection.BuildServerTypes(
  const AConnDef: IFDStanConnectionDef): Integer;
const
...
{-------------------------------------------------------------------------------}
function TFDPhysADSConnection.Build60Options(
  const AConnDef: IFDStanConnectionDef): Integer;
var
  s: String;
begin
...
{-------------------------------------------------------------------------------}
function TFDPhysADSConnection.BuildConnectionPath(
  const AConnDef: IFDStanConnectionDef): String;
var
  i: Integer;
begin
  Result := '';
  if AConnDef.HasValue(S_FD_ConnParam_Common_Server) then begin
    Result := AConnDef.Server;
    ...
    if AConnDef.HasValue(S_FD_ConnParam_Common_Port) then begin
      ...
      if AConnDef.Port <> 0 then
        Result := Result + ':' + IntToStr(AConnDef.Port);
    end;
  end;
  if AConnDef.HasValue(S_FD_ConnParam_Common_Database) or (FAliasPath <> '') then begin
    if (Result <> '') and (Copy(Result, Length(Result), 1) <> '/') and
      ...
      Result := Result + '/';
    if AConnDef.Database <> '' then
      Result := Result + AConnDef.Database
...
{-------------------------------------------------------------------------------}
function TFDPhysADSConnection.BuildADS101ConnectString(
  const AConnDef: IFDStanConnectionDef): String;
...
begin
  ...
  if AConnDef.HasValue(S_FD_ConnParam_Common_UserName) then
    Add('User ID', AConnDef.UserName);
  if AConnDef.HasValue(S_FD_ConnParam_Common_Password) then
    Add('Password', AConnDef.Password);
  ...
  if AConnDef.HasValue(S_FD_ConnParam_ADS_Advanced) then
    Result := Result + ';' + AConnDef.AsString[S_FD_ConnParam_ADS_Advanced];
end;
...
begin
  if AConnDef.HasValue(S_FD_ConnParam_ADS_Alias) then
    AConn.Lib.ResolveAlias(AConnDef.AsString[S_FD_ConnParam_ADS_Alias],
      FAliasPath, FAliasTableType)
  else begin
    FAliasPath := '';
    FAliasTableType := '';
  end;
  if Assigned(AConn.Lib.FAdsConnect101) then
    ...
    AConn.Connect60(BuildConnectionPath(AConnDef), BuildServerTypes(AConnDef),
      AConnDef.UserName, AConnDef.Password, Build60Options(AConnDef));
    if AConnDef.HasValue(S_FD_ConnParam_Common_CharacterSet) then
      if (AConn.Lib.Version > 910000000) then
...
{$ENDIF}
  ...
  if InternalGetSharedCliHandle() = nil then begin
    BaseConnect(FConnection, GetConnectionDef);

    if GetConnectionDef.NewPassword <> '' then
      InternalExecuteDirect('EXECUTE PROCEDURE sp_ModifyUserProperty(''' +
        GetConnectionDef.UserName + ''', ''USER_PASSWORD'', ''' +
        GetConnectionDef.NewPassword + '''', nil);
  end;
...
begin
  ...
  sName := String(FConnection.ConnectionPath);
...
type
  ...
  private
    FAlerter: TFDPhysADSEventAlerter;
  protected
...
begin
  if GetHandler <> nil then begin
    ...
    GetHandler.HandleEvent(oEventMsg.FName, oEventMsg.FMessage);
  end;
end;
...
{-------------------------------------------------------------------------------}
procedure TFDPhysADSCommand.SetupStatement(AStmt: TADSStatement);
var
...
begin
  ...
  if oResOpts.CmdExecTimeout <> $FFFFFFFF then
    AStmt.Timeout := (oResOpts.CmdExecTimeout + 999) div 1000;
  AStmt.ReadOnly := True;

  if not Assigned(AStmt.Lib.FAdsConnect101) then begin
  ...
  if ADSConnection.ConnectionDef.HasValue(S_FD_ConnParam_ADS_TablePassword) then begin
    oTabs := TStringList.Create;
    try
      oTabs.Delimiter := ';';
      oTabs.DelimitedText := ADSConnection.ConnectionDef.AsString[S_FD_ConnParam_ADS_TablePassword];
      for i := 0 to oTabs.Count - 1 do
        AStmt.SetTablePassword(FDNameFromIndex(oTabs, i), FDValueFromIndex(oTabs, i));
    finally
      FDFree(oTabs);
    end;
  end;
...
{-------------------------------------------------------------------------------}
function TFDPhysADSCommand.AllocStatement: TADSStatement;
begin
                                                                         
(*
  if (FOptions.UpdateOptions.LockMode = lmPessimistic) and
     (GetCommandKind in [skSelect, skSelectForLock, skSelectForUnLock, skDelete,
                         skInsert, skMerge, skUpdate]) then begin
    if ADSConnection.FSharedStmt = nil then
      ADSConnection.FSharedStmt := TADSStatement.Create(ADSConnection.FConnection,
        ADSConnection);
    Result := ADSConnection.FSharedStmt;
    Result.OwningObj := Self;
  end
  else
*)  
    Result := TADSStatement.Create(ADSConnection.FConnection, Self);
end;

{-------------------------------------------------------------------------------}
procedure TFDPhysADSCommand.ReleaseStatement(var AStmt: TADSStatement);
begin
  if ADSConnection.FSharedStmt <> AStmt then
    AStmt.Free
  else
    AStmt.OwningObj := ADSConnection;
  AStmt := nil;
end;
  ...
  for i := 0 to oParams.Count - 1 do begin
    ...
    if oParam.ParamType in [ptUnknown, ptInput, ptInputOutput] then begin
      ...
      if (iAdsParRef >= 0) and (iAdsParRef < FStmt.Params.Count) then begin
        ...
        oFmtOpts.ResolveFieldType('', oParam.DataType, oParam.FDDataType,
          oParam.Size, oParam.Precision, oParam.NumericScale, eDestFldType,
          pPar^.FSrcSize, pPar^.FSrcPrec, pPar^.FSrcScale,
...
begin
  ...
  for i := 0 to Length(FColInfos) - 1 do begin
    ...
    try
      ...
      if sKeyColumns <> '' then
        pCol^.FInPK := Pos(';' + AnsiUpperCase(pCol^.FName) + ';',
          sKeyColumns) > 0;
...
{-------------------------------------------------------------------------------}
procedure TFDPhysADSCommand.SetParamValues(AArrIndex: Integer = 0);
var
  oFmtOpts: TFDFormatOptions;
  oParams: TFDParams;
  oParam: TFDParam;
...
begin
  ...
  for i := 0 to Length(FParInfos) - 1 do begin
    ...
    if (pParInfo^.FAdsParamIndex <> -1) and
      ...
      SetParamValue(oFmtOpts, oParam, oAdsParam, pParInfo, AArrIndex);
    end;
  end;
end;

{-------------------------------------------------------------------------------}
procedure TFDPhysADSCommand.GetParamValues(AArrIndex: Integer = 0);
var
  oTab: TFDDatSTable;
  i: Integer;
  oParams: TFDParams;
...
{-------------------------------------------------------------------------------}
function TFDPhysADSCommand.InternalColInfoStart(
...
{-------------------------------------------------------------------------------}
function TFDPhysADSCommand.InternalColInfoGet(
...
begin
  ...
  if not (GetMetaInfoKind in [mkNone, mkPackages]) and
    ...
    for i := 1 to ARowsetSize do begin
      ...
      skSelectForLock:   FCursor.Lock(0);
