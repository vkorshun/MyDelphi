...
uses
  ...
  rtcDB, DB, MemTableEh, variants, RtcService, RtcResult, RtcDataSetUtils;
...
{  TSQLStringList = class (TStringList)
  ...
  TRtcQuery = class (TComponent)
  private
    FActive: Boolean;
    FCurrentUser: PRtcServiceUser;
    FSql: TStringList;
    FRtcService: TRtcService;
    FParams: TParams;
//    FClientModule: TRtcClientModule;
    FQrResult: IRtcResult;
    ...
    function GetHttpCliet: TRtcDataClient;
    procedure SetHttpClient(const Value: TRtcDataClient);
  public
    constructor Create(AOwner: TComponent;const  ACurrentUser: PRtcServiceUser);reintroduce;
...
//    property ClientModule: TRtcClientModule read FClientModule write FClientModule;
    ...
    property QrResult: IRtcResult read FQrResult;
//    property CurrentUser:PUserInfo Read FCurrentUser write FCurrentUser;
    property httpClient: TRtcDataClient read GetHttpCliet write SetHttpClient;
  end;

  TRtcSqlExecute = class (TComponent)
  private
    FCurrentUser: PRtcServiceUser;
  ...
  public
    ...
    property CurrentUser:PRtcServiceUser Read FCurrentUser write FCurrentUser;
  end;
...
begin
  Result := FQrResult.Result.asDataSet.BOF;
end;
...
//  FSql.Clear;
  if Assigned(FQrResult) then
   FQrResult.Result.Clear
end;

constructor TRtcQuery.Create(AOwner: TComponent; const ACurrentUser: PRtcServiceUser);
begin
...
//  FQrResult := TRtcValue.Create;
  FRtcService := TRtcService.Create(AOwner);
  FCurrentUser := ACurrentUser;
  FRtcService.user := ACurrentUser;
...
begin
  Result := FQrResult.Result.asDataSet.EOF;
end;
...
begin
  Result := FQrResult.Result.asDataSet.FieldByName(AFieldName)
end;

procedure TRtcQuery.First;
begin
  FQrResult.Result.asDataSet.First;
end;
...
begin
  Result := Assigned(QrResult.Result.asDataSet);
end;

function TRtcQuery.GetHttpCliet: TRtcDataClient;
begin
  Result := FRtcService.httpClient
end;

function TRtcQuery.IsEmpty: Boolean;
begin
  FQrResult.Result.asDataSet.Empty;
end;

procedure TRtcQuery.Next;
begin
  FQrResult.Result.asDataSet.Next;
end;
...
begin
  FQrResult.Result.asDataSet.Prior;
end;
...
var i: Integer;
    ...
    records: TRtcArray;
begin
  FRtcService.user := FCurrentUser;
  FRtcService.params.Clear;
  FRtcService.params.asString['query'] := FSql.Text;
  FRtcService.params.NewRecord('params');
  for i := 0 to FParams.Count-1 do
  begin
     if (FParams[i].DataType =ftDate) or (FParams[i].DataType =ftDateTime) then
          FRtcService.params.asRecord['params'].asDateTime[FParams[i].Name] := FParams[i].AsDateTime
        else
          FRtcService.params.asRecord['params'].asValue[FParams[i].Name] := FParams[i].Value;
  end;
  FQrResult := FRtcService.Execute(FPath, 'POST');
  if FQrResult.Result.isType = rtc_Record then
  begin
     if Assigned(ADataSet) then
        begin
          ADataSet.DisableControls;
          try
            fDefs := TRtcDataSetUtils.getFieldDefs(FQrResult);
            records := TRtcDataSetUtils.getDataRows(FQrResult);
              if ((ADataSet.FieldDefs.Count=0) or not isRefresh) then
              begin
                TRtcDataSetUtils.RtcJsonFieldsToDelphi( fDefs, ADataSet);
              ...
              TRtcDataSetUtils.RtcJsonRowsToDelphi(fDefs, records, ADataSet);
              ADataSet.First;
            finally
...
procedure TRtcQuery.SetHttpClient(const Value: TRtcDataClient);
begin
  FRtcService.httpClient := Value;
end;
...
begin
  ...
  begin
    ...
    begin
      Param.asWideString['username'] := FCurrentUser.userName;
      Param.asWideString['password'] := FCurrentUser.password;
...
constructor TRtcSqlExecute.Create(AOwner: TComponent; ACurrentUser: PRtcServiceUser);
begin
  inherited Create(AOwner);
  FSql:= TStringList.Create;
  FParams:= TParams.Create;
